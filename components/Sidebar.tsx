...
import { Link as LINK, LocateIcon, MapIcon, MapPin, PinIcon } from 'lucide-react'
...
export default async function Sidebar() {
    ...
    let userData = null
    let profileSlug = "guest"
    if(user){
        userData = await getUserByClerkId(user.id)
        profileSlug = user?.username ?? user?.emailAddresses[0]?.emailAddress?.split("@")[0] ?? "guest"
    }
